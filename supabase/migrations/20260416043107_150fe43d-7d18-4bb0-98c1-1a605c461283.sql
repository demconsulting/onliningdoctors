...
-- Update the RLS policy to allow patients to mark doctor_no_show
...
-- Update the notification trigger to handle doctor_no_show
...
BEGIN
  ...
  IF TG_OP = 'UPDATE' AND OLD.status IS DISTINCT FROM NEW.status THEN
    ...
    ELSIF NEW.status = 'no_show' THEN
      -- Patient no-show: notify admins, patient, and doctor
      FOR admin_id IN
      ...
      LOOP
        ...
        VALUES (
          admin_id,
          'Patient No-Show',
          'Patient no-show: ' || patient_name || ' missed appointment with Dr. ' || doctor_name || ' on ' || to_char(NEW.scheduled_at AT TIME ZONE 'UTC', 'Mon DD, YYYY at HH12:MI AM') || '.',
          'alert',
          '/admin'
        );
      END LOOP;
    ...
    ELSIF NEW.status = 'doctor_no_show' THEN
      -- Doctor no-show: notify admins, patient, and doctor
      FOR admin_id IN
        SELECT user_id FROM public.user_roles WHERE role = 'admin'
      LOOP
        INSERT INTO notifications (user_id, title, message, type, link)
        VALUES (
          admin_id,
          'Doctor No-Show',
          'Doctor no-show reported: Dr. ' || doctor_name || ' missed appointment with ' || patient_name || ' on ' || to_char(NEW.scheduled_at AT TIME ZONE 'UTC', 'Mon DD, YYYY at HH12:MI AM') || '.',
          'alert',
          '/admin'
        );
      END LOOP;
      ...
      VALUES (
        NEW.patient_id,
        'Doctor No-Show Reported',
        'Your report that Dr. ' || doctor_name || ' did not attend your appointment on ' || to_char(NEW.scheduled_at AT TIME ZONE 'UTC', 'Mon DD, YYYY at HH12:MI AM') || ' has been submitted. Our team will review this.',
        'alert',
        '/dashboard'
      );
      INSERT INTO notifications (user_id, title, message, type, link)
      VALUES (
        ...
        patient_name || ' has reported that you did not attend the appointment on ' || to_char(NEW.scheduled_at AT TIME ZONE 'UTC', 'Mon DD, YYYY at HH12:MI AM') || '. Admin has been notified.',
        'alert',
        '/doctor'
      );
    END IF;
