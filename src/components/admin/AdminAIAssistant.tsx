...
const FaqArticlesTab = () => {
  ...
  const categories = ["general", "booking", "payment", "consultation", "cancellation", "technical", "privacy", "emergency"];
  ...
  const saveArticle = async () => {
    ...
    if (editId) {
      await supabase.from("faq_articles" as any).update(form as any).eq("id", editId);
      toast({ title: "Article updated" });
    } else {
      await supabase.from("faq_articles" as any).insert({ ...form, sort_order: articles.length } as any);
    ...
    <div className="space-y-4">
      ...
      <div className="rounded-lg border border-border p-4 space-y-3">
        ...
        <Textarea
          ...
          placeholder="Article content (the AI will use this to answer questions)"
          rows={4}
        />
        ...
        <div className="flex gap-2">
          <Button onClick={saveArticle} size="sm" className="gap-1">
            {editId ? <Save className="h-3 w-3" /> : <Plus className="h-3 w-3" />}
            {editId ? "Update" : "Add"}
          </Button>
          {editId && (
      ...
      <div className="space-y-2">
        {articles.map((a: any) => (
          <div
            key={a.id}
            className="flex items-start justify-between rounded-lg border border-border p-3 gap-2"
          >
        ...
        {articles.length === 0 && (
          <p className="text-sm text-muted-foreground text-center py-6">
            No FAQ articles yet. Add your first article above.
          </p>
        )}
      </div>
    </div>
          ...
          >
            <div className="space-y-1">
              ...
              <p className="text-xs text-muted-foreground">
                ...
                {c.user_id ? c.user_id.slice(0, 8) + "..." : "Anonymous"}
              </p>
            </div>
        ...
        <DialogContent className="max-w-lg max-h-[80vh]">
          <DialogHeader>
            <DialogTitle className="text-sm font-display">
              Conversation {selectedConv?.session_id?.slice(0, 8)}...
            </DialogTitle>
          </DialogHeader>
        ...
        >
          <div className="space-y-1">
            <div className="flex items-center gap-2">
              ...
              <Badge
                variant={h.status === "pending" ? "destructive" : "default"}
                className="text-[10px]"
              >
                {h.status}
              </Badge>
...
const SettingsTab = () => {
  ...
  const save = async () => {
    setSaving(true);
    const value = { welcome_message: welcomeMessage, enabled };
    ...
    if (existing) {
      ...
      await supabase
        .from("site_content")
        .insert({ key: "ai_assistant_settings", value: value as any });
    }
    setSaving(false);
        ...
        <Tabs defaultValue="faq" className="space-y-4">
          <TabsList className="grid w-full grid-cols-4">
            <TabsTrigger value="faq" className="gap-1 text-xs">
              <BookOpen className="h-3 w-3" /> FAQ Articles
            </TabsTrigger>
            ...
            <TabsTrigger value="handoffs" className="gap-1 text-xs">
              <AlertTriangle className="h-3 w-3" /> Handoffs
            </TabsTrigger>
            <TabsTrigger value="settings" className="gap-1 text-xs">
              <Clock className="h-3 w-3" /> Settings
            </TabsTrigger>
          </TabsList>
